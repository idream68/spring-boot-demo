...
import com.springboot.demo.shiro_redis_token.utils.common.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        byte[] saveToken = tokenCache.getRedisManager().get((Constant.PREFIX_SHIRO_REFRESH_TOKEN + account).getBytes(StandardCharsets.UTF_8));
        if (saveToken != null) {
            // Redis中RefreshToken还存在，获取RefreshToken的时间戳
            String currentTimeMillisRedis = new String(saveToken, StandardCharsets.UTF_8);
            // 获取当前AccessToken中的时间戳，与RefreshToken的时间戳对比，如果当前时间戳一致，进行AccessToken刷新
            if (JwtUtil.getClaim(token, Constant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                // 获取当前最新时间戳
                String currentTimeMillis = String.valueOf(System.currentTimeMillis());
                // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(配置文件可配置refreshTokenExpireTime属性)
                tokenCache.getRedisManager().set((Constant.PREFIX_SHIRO_REFRESH_TOKEN + account).getBytes(StandardCharsets.UTF_8),
                        currentTimeMillis.getBytes(StandardCharsets.UTF_8), refreshTokenExpireTime);
